...
*Last updated: Phase 2 USB/BLE/DIN ingress foundation (`MidiIngress`) with BLE provider hook (`m5ChordBleMidiRead`) and DIN UART/provider hook (`m5ChordDinMidiRead`), per-transport MIDI IN channels (USB/BLE/DIN) with Settings + NVS/SD migration, transport external clock hooks + source gating, active-note/chord-detect foundation, Play monitor polish (compact/detailed + age/color + settings legend), BLE debug peak decay setting (off/5s/10s/30s), panic debug rotation speed setting (off/1s/2s/4s), BLE/DIN queue-drop diagnostics in MIDI Debug, outgoing MIDI TX diagnostics (bytes/rate/peak + NoteOn/NoteOff/CC counters), and initial panic-policy wiring (all-notes-off on settings-entry/ring/stop paths), plus prior Transport BPM/tap-tempo/step and Play SELECT transpose/voicing updates.*
