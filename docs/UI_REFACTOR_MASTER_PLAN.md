...
**Current shipped mapping (CoreS3 firmware):** The main Play surface still owns **tonic / key center**, **surround chord pads**, **live voicing hints**, and **play feedback**—that is the performance layer. **Transpose**, **output velocity** (gloss strip + slider tile + minus/plus), **MIDI out channel**, and **chord voicing preset** live in the **Play setup** top drawer (swipe down). **Key / mode / category** navigation uses the existing **Key picker** and **Play category** flows; **arpeggiator mode** and **velocity curve** remain on the Play touch surface and in **Settings → Seq/Arp** rather than in the drawer, to avoid duplicating dense controls.
...
**Current shipped mapping:** The drawer uses a **Feel** tab (swing, quantize, step probability, lane cycle) and a **Util** tab (per-lane MIDI channel, chord randomization depth). **BPM / clock** stay on the **Transport** drawer; per-step chord, arp, and clock edits remain on the main grid / step editor (not in the top drawer).
